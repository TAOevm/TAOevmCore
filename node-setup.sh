...
task4(){
  ...
  if [[ $totalValidator -gt 0 ]]; then
    ...
    then
      ...
      # code if not found
      echo -e '\ncd /root/TAOevmCore/' >>/etc/profile
    fi

    LINE='bash /root/TAOevmCore/node-start.sh --validator'
    if grep -Fxq "$LINE" /etc/profile
    then
      # code if found
      ...
      # code if not found
      echo -e '\nbash /root/TAOevmCore/node-start.sh --validator' >>/etc/profile
    fi
  ...
  if [[ $totalRpc -gt 0 ]]; then
    ...
    LINE='bash /root/TAOevmCore/node-start.sh --rpc'
    if grep -Fxq "$LINE" /etc/profile
    then
      # code if found
      echo -e "${ORANGE}autostart is already added"
      ...
      # code if not found
      echo -e '\nbash /root/TAOevmCore/node-start.sh --rpc' >>/etc/profile
    fi
